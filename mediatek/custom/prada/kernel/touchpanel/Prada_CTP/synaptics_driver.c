...
//      if (status & 0x20) { /* key */
        if (status & ts->fn1a_mask) { /* key */
				...
				//retval = tpd_i2c_read_data(ts->client,0x0400, &glove_val, 1);
...
/*Begin lenovo-sw wengjun1 modify for compatible of laibao and boen. 2013-1-24*/
#if 1
		tpd_i2c_read_data(client, ts->f01.query_base + 11 + 2, tp_id, 8);
		printk("[wj]the tp_id is :");
		for(i = 0; i < 8; i++)
		{
			printk("%c", tp_id[i]);
		}
		printk("\n");
		printk("[wj](int)tp_id[6] is %d.\n", (int)(tp_id[6]));
		if((int)(tp_id[6]) == 0x34)
		{
			tp_y_for_lcd = LCD_Y;
			printk("[wj]Boen have tpd_button.\n");
		}
		else if((int)(tp_id[6]) == 0x33)
		{
			//tp_y_for_lcd = (LCD_Y*TPD_BUTTON_HEIGH) /(TP_Max_Y-TPD_BUTTON_HEIGH)+LCD_Y;
			tp_y_for_lcd = 1280;
			printk("[wj]Laibao have tpd_button.\n");
		}
		else if((int)(tp_id[6]) == 0x00)
		{
			//tp_y_for_lcd = (LCD_Y*TPD_BUTTON_HEIGH) /(TP_Max_Y-TPD_BUTTON_HEIGH)+LCD_Y;
			tp_y_for_lcd = LCD_Y;
			printk("[wj]Laibao have tpd_button.\n");
		...
		{
			tp_y_for_lcd = 1280;
			printk("[wj]Use default tpd_button.\n");
		}
#endif
/*End lenovo-sw wengjun1 modify for compatible of laibao and boen. 2013-1-24*/
